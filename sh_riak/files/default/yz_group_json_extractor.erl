...
-module(yz_group_json_extractor).
-compile(export_all).
...
%% Group Visibility
extract_fields(<<"enterpriseReadPermissions">>, {struct, JSONFields}, S) ->
    Permissions = lists:foldl(extract_permissions(), [], JSONFields),
    lists:foldl(extract_element(<<"enterpriseReadPermissions">>), S, Permissions);

extract_fields(<<"userReadPermissions">>, {struct, JSONFields}, S) ->
    Permissions = lists:foldl(extract_permissions(), [], JSONFields),
    lists:foldl(extract_element(<<"userReadPermissions">>), S, Permissions);

%% Group Writeability
extract_fields(<<"enterpriseWritePermissions">>, {struct, JSONFields}, S) ->
    Permissions = lists:foldl(extract_permissions(), [], JSONFields),
    lists:foldl(extract_element(<<"enterpriseWritePermissions">>), S, Permissions);

extract_fields(<<"userWritePermissions">>, {struct, JSONFields}, S) ->
    Permissions = lists:foldl(extract_permissions(), [], JSONFields),
    lists:foldl(extract_element(<<"userWritePermissions">>), S, Permissions);
...
-spec extract_permissions() -> fun(({binary(), list()}, set()) -> set()).
extract_permissions() ->
    fun({EntityId, _}, AllShares) ->
        [EntityId | AllShares]
    end.
